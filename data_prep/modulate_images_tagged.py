...
def apply_sin_modulation_half(image, luminance, direction):
    ...
    modulated_image = np.copy(image)
    if direction == "L":
        # Calculate the width of the left half
        left_half_width = width // 2

        # Apply modulation to the left half
        modulated_image[:, :left_half_width, :] = np.clip(image[:, :left_half_width, :] * luminance, 0, 255).astype(np.uint8)
    elif direction == "R":
        # Calculate the width of the right half
        right_half_width = width // 2

        # Apply modulation to the right half
        modulated_image[:, -right_half_width:, :] = np.clip(image[:, -right_half_width:, :] * luminance, 0, 255).astype(np.uint8)

    return modulated_image



def generate_images(image, num_images, freq_left=6, freq_right=7.5):
    generated_images = []
    for i in range(num_images):
        t = i / 120
        luminance = sinusoidal(t, freq_left)
        modulated_image = apply_sin_modulation_half(image, luminance, "L")
        luminance = sinusoidal(t, freq_right)
        modulated_image = apply_sin_modulation_half(modulated_image, luminance, "R")
...
def create_gif(images, filename):
    ...
    # 120 fps
    imageio.mimsave(filename, images_rgb, duration=0.008)


def plot_sinusoidal_graph(num_captured_points=6, num_plot_points=100, frequency=12):
...
if __name__ == "__main__":
    ...
    for i, image in enumerate(random_images):
        destination_folder = f"../Pruning_Experiments/UTKFlicker_{i+1}"
        ...
        for j, img in enumerate(generated_images):
            cv2.imwrite(f'{destination_folder}/Image_{j+1}.jpg', img)
